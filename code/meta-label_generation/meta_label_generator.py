...
n_ins = 8030 #number of instances 

with open("knn_accuracy.csv") as f: 
	reader = csv.reader(f) 
	knn_acc = [row for row in reader] 

with open("nb_accuracy.csv") as f: 
	reader = csv.reader(f) 
	nb_acc = [row for row in reader] 

with open("dt_accuracy.csv") as f: 
	reader = csv.reader(f) 
...
with open("t_test_error.txt", "w", newline="") as f: 
	pass 
...
#knn vs nb 
for dataset in range(0, n_ins): #1, n_ins+1
...
with open("paired_t_test_raw_results.csv", "w", newline="") as f: 
	writer = csv.writer(f) 
...
#Store the problematic datasets 
with open("contradictions.txt", "w", newline="") as f: 
	pass 
...
for dataset in range(n_ins): 
	...
	if labels_check[t_test_temp[0]][t_test_temp[1]][t_test_temp[2]] == -1: 
		with open("contradictions.txt", "a") as f: 
			f.write(knn_acc[dataset][0] + ",") 
	else: 
		temp = []
		temp.append(knn_acc[dataset][0]) 
